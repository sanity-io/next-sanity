...
`defineInvalidateSyncTagsHandler({secret, urls})` returns a sync tag invalidate handler. It signs `{syncTags}` with `@sanity/webhook`, POSTs the body to every URL in parallel, and logs each outcome. It always calls `done()`, so a failing origin never holds a live event back. `invalidateSyncTags(syncTags, options)` is the send step on its own, for handlers you compose yourself. The package has no Next.js or React dependency.
