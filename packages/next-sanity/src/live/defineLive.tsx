...
import {expireDraftTags, revalidateSyncTags, setPerspectiveCookie} from 'next-sanity/live/server-actions'

import {resolveCookiePerspective} from './resolveCookiePerspective'
import type { DefinedLiveProps, PerspectiveType } from '#live/types'
...
export function defineLive(config: DefineSanityLiveOptions): {
  ...
  const SanityLive: React.ComponentType<DefinedSanityLiveProps> = async function SanityLive(props) {
    const {
      onChange = revalidateSyncTags,
      onChangeIncludingDrafts = expireDraftTags,
